...
class AIPostprocessor:
    ...
    def run_postprocessing(self, markdown_path: Path, ai_provider: str) -> dict:
        ...
        if ai_provider != "anthropic":
            raise ValueError(f"Unsupported AI provider: {ai_provider}. Currently supported: anthropic")
        ...
        try:
            ...
            self.logger.info(f"Successfully prepared markdown sample from {markdown_path}")
                ...
                try:
                    ...
                    if iteration == 0:
                        ...
                        if 'patterns' in current_json:
                            ...
                            self.logger.info(f"Found {pattern_count} potential issues in first analysis")
                    else:
                        ...
                        if 'patterns' in combined_json:
                            ...
                            self.logger.info(f"Total patterns after round {iteration + 1}: {total_patterns}")
                        
                except json.JSONDecodeError as e:
                    self.logger.error(f"Failed to parse AI response in round {iteration + 1}: {e}")
                    self.logger.error("This may indicate an issue with the AI service or prompt")
                    raise
            ...
            if success:
                ...
                self.logger.info("Markdown content has been improved based on AI analysis")
            else:
            ...
            self.logger.error("AI postprocessing failed - original markdown content unchanged")
            raise
        ...
        try:
            ...
            with open(prompt_path, 'r', encoding='utf-8') as f:
                ...
                if not content.strip():
                    raise ValueError("prompt.txt is empty - cannot proceed with AI analysis")
        ...
        try:
            ...
            with open(markdown_path, 'r', encoding='utf-8') as f:
                ...
                if len(words) > max_words:
                    ...
                    self.logger.info(f"Sampled {len(sampled_words)} words from {len(words)} total")
